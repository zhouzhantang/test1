                    ...
                    void *p2, void *p3, int prio, u32_t options, s32_t delay)
{
    ...
    if (!stack)
    { // Use dynamic stack if no static one provided
        int ret;

        ret = aos_task_new_ext(&(new_thread->task), task_name, (task_entry_t)entry, p2,
                               stack_size, prio);
        if (ret)
        {
    ...
    { // Use static statck if provided
        ...
        return krhino_task_create(task_obj, task_name, p2, prio, 0,
                                  (cpu_stack_t *)stack,
                                  stack_size,
                                  (task_entry_t)entry,
                                  delay == 0 ? 1 : 0);
