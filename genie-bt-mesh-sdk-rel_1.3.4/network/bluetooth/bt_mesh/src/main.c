...
bool bt_mesh_gatt_is_user_enabled(void)
{
    return gatt_user_en;
}
...
void bt_mesh_setup(u32_t seq, u16_t addr)
{
    ...
    if (IS_ENABLED(CONFIG_BT_MESH_GATT_PROXY) &&
        (bt_mesh_gatt_proxy_get() != BT_MESH_GATT_PROXY_NOT_SUPPORTED) && bt_mesh_gatt_is_user_enabled())
    {
                      ...
                      u16_t addr, const u8_t dev_key[16])
{
    ...
    if (err)
    {
        if (IS_ENABLED(CONFIG_BT_MESH_PB_GATT) && bt_mesh_gatt_is_user_enabled())
        {
...
int bt_mesh_prov_enable(bt_mesh_prov_bearer_t bearers)
{
    ...
    if (IS_ENABLED(CONFIG_BT_MESH_PB_GATT) &&
        (bearers & BT_MESH_PROV_GATT) && bt_mesh_gatt_is_user_enabled())
    {
        bt_mesh_proxy_prov_enable();
        bt_mesh_adv_update();
    }
...
bool bt_mesh_is_suspended(void)
{
    return suspended;
}
...
int bt_mesh_suspend(bool force)
{
    ...
    if (bt_mesh_is_suspended())
    {
        BT_INFO("already in suspended stat, return");
        return 0;
    }
    ...
    bt_mesh_multi_adv_event_stop();
    bt_mesh_proxy_multi_adv_stop();
#endif
...
int bt_mesh_resume(void)
{
    ...
    if (!bt_mesh_is_suspended())
    {
        BT_INFO("no in suspended stat, return");
        return 0;
    }
...
#ifdef CONFIG_BT_MESH_MULTIADV
    bt_mesh_multi_adv_event_start();
    bt_mesh_proxy_multi_adv_start();
#endif
