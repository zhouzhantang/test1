...
/*[Genie begin] add by wenbing.cwb at 2021-10-16*/
static struct bt_mesh_app_key *app_key_alloc(u16_t app_idx)
{
    ...
    return &bt_mesh.app_keys[app_idx];
}
/*[Genie end] add by wenbing.cwb at 2021-10-16*/
...
static s8_t genie_mesh_setup(u16_t key_app_idx)
{
    ...
    for (i = 0; i < elem_cnt; i++)
    {
        ...
        for (j = 0; j < elem->vnd_model_count; j++)
        {
            ...
            mod_bind(mod, key_app_idx);
            //BT_DBG(">>>model %04x %04x app_key[%02x]<<<", elem->vnd_models[j].vnd.company, elem->vnd_models[j].vnd.id, status);
            memcpy(mod->groups, g_sub_list, sizeof(g_sub_list));
        }
    }
                        ...
                        struct net_buf_simple *buf)
{
    ...
    genie_mesh_setup(key_app_idx);

    if (key_app_idx == 0)
    {
        genie_event(GENIE_EVT_SDK_APPKEY_ADD, &status);
    }
    else if (key_app_idx == 1)
    {
        genie_event(GENIE_EVT_SDK_APPKEY1_ADD, &status);
    }
}
