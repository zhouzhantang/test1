...
u16_t *bt_mesh_model_find_group(struct bt_mesh_model *mod, u16_t addr)
{
    ...
    /*[Genie begin] add by lgy at 2021-06-07*/
...
#ifdef MESH_DEBUG_RX
    SYS_LOG_INF("RX Info");
    MESH_MSG_RX("RSSI: %d", rx->rssi);
    MESH_MSG_RX("TTL: %d AppIdx: 0x%04x", rx->ctx.recv_ttl, rx->ctx.app_idx);
...
#ifdef MESH_DEBUG_TX
    SYS_LOG_INF("TX Info");
    MESH_MSG_TX("TTL: %d AppIdx: 0x%04x", tx->ctx->send_ttl, tx->ctx->app_idx);
...
{
    BT_INFO("get appkey id:%d", appkey_id);
    return appkey_id;
}

void bt_mesh_model_set_appkey_id(u16_t key_id)
{
    BT_INFO("set appkey id:%d", key_id);
    appkey_id = key_id;
}
