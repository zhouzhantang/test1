					...
					struct net_buf_simple *buf)
{
	...
	for (i = 0; i < array_size; i++)
	{
		...
		{
			BT_WARN("net id not match(%d-%d)", key->net_idx, rx->sub->net_idx);
			continue;
		}
		...
		if (AID(&hdr) != keys->id)
		{
			BT_DBG("AppKey id not match(%d-%d)", AID(&hdr), keys->id);
			continue;
		}
						   ...
						   struct net_buf_simple *buf)
{
	...
	{
		BT_DBG("Ignoring heartbeat to non-subscribed destination");
		return 0;
	}
								  ...
								  const u64_t *seq_auth)
{
	...
	for (i = 0; i < ARRAY_SIZE(seg_rx); i++)
	{
		...
		if (rx->in_use)
		{
			BT_DBG("Duplicate SDU from src 0x%04x",
					net_rx->ctx.addr);
