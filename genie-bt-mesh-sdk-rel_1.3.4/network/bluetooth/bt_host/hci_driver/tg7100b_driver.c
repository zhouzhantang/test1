...
#define CONFIG_BT_RXSTACK_SIZE 1024
static BT_STACK_NOINIT(rx_thread_stack, CONFIG_BT_RXSTACK_SIZE);
...
extern void le_p256_pub_key(struct net_buf *buf);
extern void le_gen_dhkey(struct net_buf *buf);
extern void clear_ecc_events(struct net_buf *buf);
extern int hci_driver_register(int idx);
static int h4_send(struct net_buf *buf)
{
    int ret;
    ...
    ret = hci_send((yoc_dev_t *)hci_h4.dev, buf->data, buf->len);
...
static int h4_open(void)
{
    ...
    ret =  k_thread_create(&rx_thread_data, rx_thread_stack, sizeof(rx_thread_stack), rx_thread,
                           "hci_rx", NULL, NULL, CONFIG_BT_RX_PRIO, 0, K_NO_WAIT);

    if (ret) {
        return ret;
    }

    hci_h4.dev = (dev_t *)hci_open_id(hci_h4.dev_name, 0);
