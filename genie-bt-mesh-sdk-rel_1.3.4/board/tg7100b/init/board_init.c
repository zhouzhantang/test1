        ...
        [HAL_PARTITION_KV] =
            {
                ...
                .partition_start_addr = 0x11007000,
        ...
        [HAL_PARTITION_OTA_TEMP] =
            {
                ...
                .partition_start_addr = 0x11021000,
...
void init_config(void)
{
    ...
    *(uint32 *)(0x1fff101c) = 0x1fff0c00; // set ROM code pGlobal_config
    ...
    // LL switch setting
    pGlobal_config[LL_SWITCH] = LL_DEBUG_ALLOW | SLAVE_LATENCY_ALLOW | LL_WHITELIST_ALLOW | SIMUL_CONN_ADV_ALLOW | SIMUL_CONN_SCAN_ALLOW | GAP_DUP_RPT_FILTER_DISALLOW; //RC32_TRACKINK_ALLOW
    ...
    // sleep time, in us
    pGlobal_config[MAX_SLEEP_TIME] = 8 * 60 * 1000 * 1000; //set max sleep time to 8min
...
void hal_rfphy_init(void)
{
    ...
    *(volatile uint32_t *)0x4000f00c = 0x36da4db6; //P10 - P19 pull down
    *(volatile uint32_t *)0x4000f010 = 0x36da6db6; //P20 - P29 pull down
    *(volatile uint32_t *)0x4000f014 = 0xb0c3edb6; //P30 - P34 pull donw

    //========= UART RX Pull up
    phy_gpio_pull_set(P15, PULL_DOWN);
...
void __attribute__((weak)) board_init(void)
{
    ...
    //指定管脚复用为UART功能
    //drv_pinmux_config(P1, UART_TX);
    //drv_pinmux_config(P15, UART_RX);
}
