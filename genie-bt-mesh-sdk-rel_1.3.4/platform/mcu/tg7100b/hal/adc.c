...
enum {
    ADC_INTERNAL_CAP_DISABLE = 0,
    ADC_INTERNAL_CAP_ENABLE = 1,
};
...
int32_t hal_adc_value_get(adc_dev_t *adc, void *output, uint32_t timeout)
{
    ...
    //ADC_INTERNAL_CAP_ENABLE;ADC_INTERNAL_CAP_DISABLE
    sconfig.enable_link_internal_voltage = ADC_INTERNAL_CAP_DISABLE;
    if (sconfig.enable_link_internal_voltage == ADC_INTERNAL_CAP_ENABLE) {
        sconfig.link_internal_voltage_channel = adc->port;
    }
