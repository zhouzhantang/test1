...
int32_t hal_pwm_init(pwm_dev_t *pwm)
{
    ...
    pwm_handle_t handle = csi_pwm_initialize(0);
    ...
    if(pwm->config.duty_cycle != 0)
        ret = csi_pwm_config((pwm_handle_t)pwm->priv, pwm->port, period_us, pwm->config.duty_cycle);
