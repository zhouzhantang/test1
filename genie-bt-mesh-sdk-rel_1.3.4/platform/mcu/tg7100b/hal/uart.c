...
int32_t hal_uart_send(uart_dev_t *uart, const void *data, uint32_t size, uint32_t timeout)
{
    ...
    ret = aos_sem_wait(&uart_list[uart->port].tx_sem, timeout);
    return ret;
}
...
uint8_t uart_it_buf[512];
uint8_t uart_it_index;

uint8_t *recv_bbb = NULL;
uint8_t wait_uart_rx_over;
                         ...
                         uint32_t *recv_size, uint32_t timeout)
{
	...
	recv_bbb = (uint8_t *)data;
	//printk("recv_bbb: %c   %d\r\n",*recv_bbb,*recv_size);
	uart_it_buf[uart_it_index] = *recv_bbb;
	uart_it_index++;
    wait_uart_rx_over=0;
	if(uart_it_index>=256){
		uart_it_index = 0;
	}
...
{
    aos_sem_free(&uart_list[uart->port].tx_sem);
    aos_sem_free(&uart_list[uart->port].rx_sem);
