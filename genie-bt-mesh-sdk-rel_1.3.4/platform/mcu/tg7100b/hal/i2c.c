...
int32_t hal_i2c_init(i2c_dev_t *i2c)
{
    ...
    handle[i2c->port] = csi_iic_initialize(i2c->port, i2c_cb);

    if (handle[i2c->port] == NULL) {
        return -1;
    }
    ...
    return  csi_iic_config(handle[i2c->port], mode, speed, addr_mode, i2c->config.dev_addr);
}
                            ...
                            uint16_t size, uint32_t timeout)
{
    ...
    int ret = csi_iic_master_send((iic_handle_t)(handle[i2c->port]), dev_addr, data, size, 0);
    ...
    if (ret != 0) {
        printf("i2c:%d  master send timeout\n", i2c->port);
    }
...
{
    aos_mutex_lock(&mutex[i2c->port], AOS_WAIT_FOREVER);
    int ret = csi_iic_master_receive((iic_handle_t)(handle[i2c->port]), dev_addr, data, size, 0);
    ...
    if (ret != 0) {
        printf("i2c:%d master recv timeout\n", i2c->port);
    }
...
{
    aos_mutex_lock(&mutex[i2c->port], AOS_WAIT_FOREVER);
    int ret = csi_iic_slave_send((iic_handle_t)(handle[i2c->port]), data, size);
    ...
    if (ret != 0) {
        printf("i2c:%d slave send timeout\n", i2c->port);
    }
...
{
    aos_mutex_lock(&mutex[i2c->port], AOS_WAIT_FOREVER);
    int ret = csi_iic_slave_receive((iic_handle_t)(handle[i2c->port]), data, size);
    ...
    if (ret != 0) {
        printf("i2c:%d slave rcv timeout\n", i2c->port);
    }
                          ...
                          uint32_t timeout)
{
    aos_mutex_lock(&mutex[i2c->port], AOS_WAIT_FOREVER);
    int ret  = 0;
    //uint16_t tx_len = size;
    uint8_t *buf = aos_malloc(mem_addr_size + size);
    ...
    ret = csi_iic_master_send((iic_handle_t)(handle[i2c->port]), dev_addr, buf, size+mem_addr_size, 0);
    ...
    /*while (tx_len > 0) {
        ret = csi_iic_master_send((iic_handle_t)(handle[i2c->port]), dev_addr, buf, 1, 0);
                         ...
                         uint32_t timeout)
{
    ...
    int ret = csi_iic_master_send((iic_handle_t)(handle[i2c->port]), dev_addr, &mem_addr, mem_addr_size, 0);
    ...
    ret = csi_iic_master_receive((iic_handle_t)(handle[i2c->port]), dev_addr, data, size, 0);
...
int32_t hal_i2c_finalize(i2c_dev_t *i2c)
{
    ...
    return csi_iic_uninitialize((iic_handle_t)(handle[i2c->port]));
}
