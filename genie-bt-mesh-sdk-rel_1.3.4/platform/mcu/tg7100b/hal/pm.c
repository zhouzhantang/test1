...
extern uint64_t aos_kernel_tick2ms(uint32_t ticks);
extern uint64_t aos_kernel_ms2tick(uint32_t ms);
...
#if defined CONFIG_WDT && defined CONFIG_PM_SLEEP
...
static pm_wakeup_by_io_cb g_wakeup_cb = NULL;

#if defined CONFIG_PM_SLEEP
extern void hal_ret_sram_enable(uint8_t sram_index);
