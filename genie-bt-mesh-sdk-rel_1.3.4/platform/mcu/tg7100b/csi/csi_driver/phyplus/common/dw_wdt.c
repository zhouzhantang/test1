...
/**
  \brief       config if the WDT need auto feed.
  \param[in]   handle wdt handle to operate.
  \param[in]   auto_feed     Pointer to the Value.
  \return      \ref execution_status
*/
int32_t csi_wdt_config_feed(int32_t idx, wdt_feed_e auto_feed)
{
    if (idx < 0 || idx >= CONFIG_WDT_NUM) {
        return ERR_WDT(DRV_ERROR_PARAMETER);
    }
    ...
    dw_wdt_priv_t *wdt_priv = &wdt_instance[idx];
    if (!wdt_priv->base || !wdt_priv->irq) {
        return ERR_WDT(DRV_ERROR_PARAMETER);
    }
    wdt_priv->auto_feed = auto_feed;
    return 0;
}
