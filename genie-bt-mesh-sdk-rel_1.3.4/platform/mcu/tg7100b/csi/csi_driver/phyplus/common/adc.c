...
uint32_t adc_data[MAX_ADC_SAMPLE_SIZE];
...
int8_t drv_adc_pin2channel(uint8_t port)
{
    ...
    case P11:
    {
        adc_channel = ADC_CH1N_P11;
    }
    break;
    case P12:
    {
        adc_channel = ADC_CH1P_P12;
    }
    break;
    case P13:
    {
        adc_channel = ADC_CH2N_P13;
    }
    break;
    case P14:
    {
        adc_channel = ADC_CH2P_P14;
    }
    break;
    case P15:
    {
        adc_channel = ADC_CH3N_P15;
    }
    break;
    case P20:
    {
        adc_channel = ADC_CH3P_P20;
    }
    break;
    }
...
static int read_data_one_channel_n(adc_handle_t handle, uint32_t *data, uint32_t num)
{
    ...
    while (i < num) {
        ...
        data[i] = (uint32_t)(phy_adc_value_cal(ch, fdata, 1, adc_priv->cfg.is_high_resolution, adc_priv->cfg.is_differential_mode));
        i++;
        if (i >= num) {
            break;
        }
...
static int read_multiple_channel_once(adc_handle_t handle, uint32_t *data, uint32_t *buf_len)
{
    ck_adc_priv_t *adc_priv = handle;
    ...
    while (i < 8) {
        if (channel_mask & BIT(i)) {
            if (j % 2) {
                ...
                data[j] = (uint32_t)(phy_adc_value_cal(i, fdata, 1, adc_priv->cfg.is_high_resolution, adc_priv->cfg.is_differential_mode));
            } else {
        ...
        uint32_t ch_num)
{
    ...
    while (i < ADC_FIFO_LENGTH && *read_len) {
        read_multiple_channel_once(handle, data, read_len);
        data += ch_num;
        i++;
    }
    ...
    for(int i = 0;i < 1000; i++){
        ...
        if (GET_IRQ_STATUS & BIT(sel_ch)) {
            for (int n = 0; n < (MAX_ADC_SAMPLE_SIZE - 31); n++) {
                adc_data[n] = (read_reg(ADC_CH_BASE + (sel_ch * 0x80) + ((n+3) * 4))&0xfff);
            }
