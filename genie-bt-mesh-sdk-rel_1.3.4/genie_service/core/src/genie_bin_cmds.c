...
static int genie_bin_cmd_update_group_addr(uint8_t *addr_list, int addr_len)
{
    ...
    for (index = 0; index < addr_len; index++)
    {
        if ((index % 2) == 0)
        {
            ...
            {
                if ((group_addr_list[index] != 0) || (group_addr_list[index + 1] != 0)) //For compatiable with westlack one
                {
                    return -2;
                }
            }
    ...
    case GENIE_CTL_TEST_MODE:
        if (len == 8)
        {
            if (p_u8[5] == 0x00)
            {
                ...
                if (genie_sal_ble_get_rssi(g_rssi_mac, (genie_sal_ble_get_rssi_cb)get_ble_rssi_cb) < 0)
                {
