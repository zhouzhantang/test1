...
static int __item_del_all_cb(kv_item_t *item, const char *ex_prefix)
{
    if (item->hdr.key_len > 0)
    {
        ...
        {
            char *key = NULL;
            if (item->hdr.key_len < strlen(ex_prefix))
            {
                kv_item_del(item, KV_DELETE_REMOVE);
                return RES_CONT;
            }
            ...
            memset(key, 0, item->hdr.key_len + 1);
            kv_os_partition_read(item->pos + ITEM_HDR_SIZE, key, item->hdr.key_len);
            if (strncmp(key, ex_prefix, strlen(ex_prefix)) != 0)
            {
                kv_item_del(item, KV_DELETE_REMOVE);
            }
            kv_os_free(key);
...
{
    int i, ret;
    if ((ret = kv_os_mutex_lock(&(g_kv_mgr.mutex), KV_WAIT_FOREVER)) != RES_OK)
    {
        return ret;
    }
    ...
    for (i = 0; i < BLK_NUMS; i++)
    {
        kv_item_traverse(__item_del_all_cb, i, ex_prefix);
    }
...
static int _kv_getx_cmd(const char *key)
{
    int ret = 0;
    int index = 0;
    ...
    else
    {
        ...
        if (_is_printable_string((char *)buffer, item->hdr.val_len))
        {
            aos_cli_printf("\r\nkey:%s vaule[%d]:%s\r\n", key, item->hdr.val_len, (char *)buffer);
        }
        else
        {
            aos_cli_printf("\r\nkey:%s binary value[%d] is\r\n", key, item->hdr.val_len);
